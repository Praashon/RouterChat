...
export function MessageBubble({ 
  ...
  return (
    <div className={`w-full py-4 sm:py-6 flex px-3 sm:px-4 md:px-6 ${isUser ? "bg-background" : "bg-zinc-50/50 dark:bg-zinc-900/20 border-y border-border/40"}`}>
      <div className="max-w-3xl mx-auto w-full flex gap-3 sm:gap-5">
        ...
        <div className={`flex-1 min-w-0 ${isStreaming ? 'animate-streaming-message' : ''}`}>
            ...
            <div className="chat-markdown">
              <ReactMarkdown
                ...
                components={{
                  // Headings
                  h1: ({children}) => (
                    <h1 className="text-[22px] font-bold tracking-tight text-foreground mt-6 mb-3 pb-2 border-b border-border/40 font-heading first:mt-0">{children}</h1>
                  ),
                  h2: ({children}) => (
                    <h2 className="text-[18px] font-bold tracking-tight text-foreground mt-5 mb-2.5 font-heading first:mt-0">{children}</h2>
                  ),
                  h3: ({children}) => (
                    <h3 className="text-[16px] font-semibold tracking-tight text-foreground mt-4 mb-2 font-heading first:mt-0">{children}</h3>
                  ),
                  h4: ({children}) => (
                    <h4 className="text-[15px] font-semibold text-foreground mt-3 mb-1.5 font-heading first:mt-0">{children}</h4>
                  ),

                  // Paragraphs
                  p: ({children}) => (
                    <p className="text-[15px] leading-[1.75] text-foreground/90 my-2.5 first:mt-0 last:mb-0">{children}</p>
                  ),
                  ...
                  // Unordered lists
                  ul: ({children}) => (
                    <ul className="my-3 ml-1 space-y-1.5 list-none first:mt-0">{children}</ul>
                  ),

                  // Ordered lists
                  ol: ({children, start}) => (
                    <ol start={start} className="my-3 ml-1 space-y-1.5 list-none counter-reset-list first:mt-0">{children}</ol>
                  ),

                  // List items
                  li: ({children, ordered, index}: any) => (
                    <li className="relative pl-6 text-[15px] leading-[1.7] text-foreground/90">
                      <span className="absolute left-0 top-0 select-none text-zinc-400 dark:text-zinc-500 text-[14px] font-medium" aria-hidden="true">
                        {ordered !== undefined && typeof index === 'number' ? `${index + 1}.` : '\u2022'}
                      </span>
                      {children}
                    </li>
                  ),
                  ...
                  thead: ({children}) => (
                    <thead className="bg-zinc-100/80 dark:bg-zinc-800/50">{children}</thead>
                  ),
                  th: ({children}) => (
                    <th className="px-4 py-2.5 text-left font-semibold text-[13px] text-foreground tracking-tight border-b border-zinc-200/60 dark:border-zinc-700/60">{children}</th>
                  ),
                  td: ({children}) => (
                    <td className="px-4 py-2.5 text-[14px] text-foreground/85 border-b border-zinc-100 dark:border-zinc-800/40">{children}</td>
                  ),
                  tr: ({children}) => (
                    <tr className="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/20 transition-colors">{children}</tr>
                  ),
                  ...
                  code({node, inline, className, children, ...props}: any) {
                     ...
                     return !inline && match ? (
                       ...
                       <code className="bg-zinc-100 dark:bg-zinc-800 px-1.5 py-0.5 rounded-md text-[13px] font-mono before:content-none after:content-none border border-border/40 text-zinc-800 dark:text-zinc-200" {...props}>
                         {children}
                       </code>
                     )
                  }
